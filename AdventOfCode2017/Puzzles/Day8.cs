...
namespace AdventOfCode2017.Puzzles {
    ...
    public sealed class Day8 : iPuzzle {
        ...
        private readonly string _testInput = @"b inc 5 if a > 1
        a inc 1 if b < 5
        c dec -10 if a >= 1
        c inc -20 if c == 10";

        private readonly string _input = @"o inc 394 if tcy >= -3
        s inc -204 if s <= -5
        keq inc -34 if keq > -6
        ip inc 762 if ip == 4
        zr inc -644 if ip >= 6
        ...
        public void Part1() {
            List<string> instructions = this.DecodeInstructions();
            ...
            foreach (var instructionString in instructions) {
                ...
                if (instruction.IsConditionMet(ref registers)) {
                    instruction.ModifyRegistry(ref registers);
                }
            }

            int largestRegister = registers.Values.ToList().Max(x => x.Value);
            Console.WriteLine("Part 1: {0}", largestRegister);
        }

        public void Part2() {
            int largestRegister = -1;
            List<string> instructions = this.DecodeInstructions();
            Dictionary<string, Register> registers = new Dictionary<string, Register>();
            ...
            foreach (var instructionString in instructions) {
                ...
                // do a check for the largest! (part 2 specific)
                int currentLargest = registers.Values.ToList().Max(x => x.Value);
                if (currentLargest > largestRegister) {
                    largestRegister = currentLargest;
                }
                // part 2 specific end
            }

            Console.WriteLine("Part 2: {0}", largestRegister);
        }

        private List<string> DecodeInstructions(bool useTestInput = false) {
            string input = (useTestInput) ? _testInput : _input;
            return input.Split(new[] { Environment.NewLine }, StringSplitOptions.None).Select(x => x.Trim()).Where(y => !String.IsNullOrWhiteSpace(y)).ToList();
        }
        ...
        private sealed class Register {
            ...
            public Register(string name, int initalValue = 0) {
                Name = name;
                Value = initalValue;
            }
        }
        ...
        private sealed class Instruction {
            ...
            public Operation TargetOperation { get; private set; }
            public int OperationValue { get; private set; }

            public string ConditionTarget { get; private set; }
            public Condition IfCondition { get; private set; }
            public int ConditionValue { get; private set; }

            public Instruction(string instruction) {
                // we have to decode the instruction!
                // format? 'b inc 5 if a > 1'
                // array?   0  1  2  3 4 5 6
                string[] aInstruction = instruction.Split(' ');
                // take care of the obvious?
                TargetRegister = aInstruction[0];
                ConditionTarget = aInstruction[4];

                TargetOperation = this.GetOperation(aInstruction[1]);
                IfCondition = this.GetCondition(aInstruction[5]);

                OperationValue = int.Parse(aInstruction[2]);
                ConditionValue = int.Parse(aInstruction[6]);
            }

            public bool IsConditionMet(ref Dictionary<string, Register> registry) {
                this.DoRegistryAvailableCheck(ref registry);

                Register theConditionTarget = registry[this.ConditionTarget]; // should not throw as we checked above!
                int conditionTargetValue = theConditionTarget.Value;

                switch (this.IfCondition) {
                    case Condition.GreaterThan:
                        return (conditionTargetValue > this.ConditionValue);
            ...
            public void ModifyRegistry(ref Dictionary<string, Register> registry) {
                this.DoRegistryAvailableCheck(ref registry); // be safe!
                
                if (this.TargetOperation == Operation.Inc) {
                    registry[this.TargetRegister].Value += this.OperationValue;
                }
                else {
                    registry[this.TargetRegister].Value -= this.OperationValue;
                }
            ...
            private Operation GetOperation(string value) {
                value = value.ToLower();
                return (value == "inc") ? Operation.Inc : Operation.Dec;
            }
            ...
            private void DoRegistryAvailableCheck(ref Dictionary<string, Register> registry) {
                if (!registry.ContainsKey(this.ConditionTarget)) {
                    // the registery does not exist, so we need to create it
                    registry.Add(this.ConditionTarget, new Register(this.ConditionTarget));
                }

                if (!registry.ContainsKey(this.TargetRegister)) {
                    // the target registry does not exist, so we need to create it!
                    registry.Add(this.TargetRegister, new Register(this.TargetRegister));
                }
            }
        }
    }
