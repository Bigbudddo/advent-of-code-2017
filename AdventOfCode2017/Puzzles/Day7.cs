...
namespace AdventOfCode2017.Puzzles {
    ...
    public sealed class Day7 : iPuzzle {
        ...
        /*
        private readonly string _input = @"pbga (66)
        xhth (57)
        ebii (61)
        havc (66)
        ktlj (57)
        ...
        public void Part1() {
            List<Program> input = this.BuildInput();
            // work our way to the bottom of the pile and find the bottom program!
            Program baseProgram = input.First();
            do {
                baseProgram = baseProgram.Parent;
            } while (baseProgram.Parent != null);

            Console.WriteLine("Part 1: {0}", baseProgram.Name);
        }

        public void Part2() {
            List<Program> input = this.BuildInput();
            // work our way to the bottom to get the base program
            Program baseProgram = input.First();
            do {
                baseProgram = baseProgram.Parent;
            } while (baseProgram.Parent != null);
            // find the unbalanced child program
            int weight = 0;
            while (!baseProgram.IsBalanced) {
                Tuple<Program, int> results = baseProgram.GetUnbalancedChildProgram();

                weight = results.Item2;
                baseProgram = results.Item1;
            }
            // get the difference?
            int difference = baseProgram.Weight + (weight - baseProgram.TotalWeight);
            Console.WriteLine("Part 2: {0}", difference);
        }

        private List<Program> BuildInput() {
            List<Program> returnValues = new List<Program>();
            List<string> programs = _input.Split(new[] { Environment.NewLine }, StringSplitOptions.None).Select(x => x.Trim()).Where(y => !String.IsNullOrWhiteSpace(y)).ToList();
           
            foreach (var program in programs) {
                Program builtProgram = null;
                string[] aProgramData = program.Split(' ');

                string name = aProgramData[0];
                int weight = int.Parse(new string(aProgramData[1].Skip(1).TakeWhile(x => x != ')').ToArray()));

                if (aProgramData.Length == 2) {
                    // just a program, with a weight and no tower inputs
                    builtProgram = new Program(name, weight, false, new List<string>().ToArray());
                }
                else {
                    // program with a weight, and other programs in the tower
                    // so, index 2 will have this '->'
                    string[] childNames = aProgramData.Skip(3).Select(x => new string(x.TakeWhile(y => y != ',').ToArray())).ToArray();
                    builtProgram = new Program(name, weight, false, childNames);
                }

                if (builtProgram != null) {
                    returnValues.Add(builtProgram);
                }
            }

            // now we need to build a 'tree' network
            foreach (var program in returnValues) {
                program.GetChildrenByName(returnValues);
            }
        ...
        private sealed class Program {
            ...
            public List<string> ChildProgramNames { get; private set; }
            public List<Program> ChildPrograms { get; private set; }

            public bool IsBalanced {
                get {
                    return (ChildPrograms.GroupBy(x => x.TotalWeight)).Count() == 1;
                }
            }

            public int TotalWeight {
                get {
                    return (ChildPrograms.Sum(x => x.TotalWeight)) + this.Weight;
                }
            }

            public Program(string name, int weight, bool isHoldingDisk, params string[] childNames) {
                Name = name;
                Weight = weight;
                ChildProgramNames = childNames.ToList();

                Parent = null;
                ChildPrograms = new List<Program>();
            }
            ...
            public Tuple<Program, int> GetUnbalancedChildProgram() {
                var weightGroupings = this.ChildPrograms.GroupBy(x => x.TotalWeight);

                return new Tuple<Program, int>(
                    weightGroupings.First(x => x.Count() == 1).First(),
                    weightGroupings.First(x => x.Count() > 1).Key
                );
            }
        }
    }
